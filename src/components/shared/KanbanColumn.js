...
export default function KanbanColumn({
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="flex flex-col gap-4 min-h-[600px] rounded-[2.5rem] border-2 border-dashed border-white/[0.03] p-3 bg-white/[0.01] transition-colors hover:bg-white/[0.02]">
        {tasks.length > 0 ? (
          tasks.map((task) => (
            <TaskCard
              key={task._id}
